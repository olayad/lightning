...
{
	if (amount_msat_greater(delta_fee, cons->fee_budget))
		return false;
						     ...
						     struct payment *p)
{
	...
	reason = tal_fmt(
	    tmpctx, "Error computing a route to %s: %.*s (%d)",
			     ...
			     const jsmntok_t *toks, struct payment *p)
{
	...
	payment_fail(p, p->result->message);
	return command_still_pending(cmd);
}
